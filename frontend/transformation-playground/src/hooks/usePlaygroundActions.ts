...
export const usePlaygroundActions = () => {
  ...
  const addInputDocument = useCallback((name: string, content: string) => {
  ...
  const addTransformation = useCallback((name: string, script: string) => {
  ...
  const reorderTransformation = useCallback((fromIndex: number, toIndex: number) => {
    dispatch({ 
